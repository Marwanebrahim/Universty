...
public class RigesterSystem {
    ...
    // Reports and Listings (Modified to return strings for GUI)
    public String getDepartmentsList() {
        StringBuilder sb = new StringBuilder("Departments:\n");
        Node<Department> temp = departments.head;
        if (temp == null) {
            sb.append("No departments available.\n");
        }
        while (temp != null) {
            sb.append("Department Name: ").append(temp.data.getDepartmentName()).append("\n");
            temp = temp.next;
        }
        return sb.toString();
    }

    public String getCoursesListInDepartment(String departmentName) {
        Department dept = searchDepartment(departmentName);
        if (dept != null) {
            StringBuilder sb = new StringBuilder("Courses in ").append(departmentName).append(":\n");
            // Iterate through the courses HashTable
            for (int i = 0; i < dept.getCourses().getTable().length; i++) {
                SingleLinkedList<Courses> bucket = dept.getCourses().getTable()[i];
                Node<Courses> current = bucket.head;
                while (current != null) {
                    sb.append(current.data.toString()).append("\n");
                    current = current.next;
                }
            }
            if (sb.length() == ("Courses in " + departmentName + ":\n").length()) {
    ...
    public String getStudentsInCourse(Department department, String courseID) {
        Courses course = department.searchCourse(courseID);
        if (course != null) {
            StringBuilder sb = new StringBuilder("Enrolled Students in ").append(courseID).append(":\n");
            Node<Student> temp = course.getEnrolledStudents().head;
            if (temp == null) {
    ...
    // Added methods for GUI lists with full implementation
    public String getStudentsListInDepartment(String departmentName) {
        Department dept = searchDepartment(departmentName);
        if (dept != null) {
            StringBuilder sb = new StringBuilder("Students in " + departmentName + ":\n");
            // Iterate through the students HashTable
            for (int i = 0; i < dept.getStudents().getTable().length; i++) {
                SingleLinkedList<Student> bucket = dept.getStudents().getTable()[i];
                Node<Student> current = bucket.head;
                while (current != null) {
                    sb.append(current.data.toString()).append("\n");
                    current = current.next;
                }
            }
            if (sb.length() == ("Students in " + departmentName + ":\n").length()) {
    ...
    public String getInstructorsListInDepartment(String departmentName) {
        Department dept = searchDepartment(departmentName);
        if (dept != null) {
            StringBuilder sb = new StringBuilder("Instructors in " + departmentName + ":\n");
            // Iterate through the instructors HashTable
            for (int i = 0; i < dept.getInstructors().getTable().length; i++) {
                SingleLinkedList<Instructor> bucket = dept.getInstructors().getTable()[i];
                Node<Instructor> current = bucket.head;
                while (current != null) {
                    sb.append(current.data.toString()).append("\n");
                    current = current.next;
                }
            }
            if (sb.length() == ("Instructors in " + departmentName + ":\n").length()) {
                sb.append("No instructors available.\n");
            }
            return sb.toString();
        }
        return "Department not found.";
